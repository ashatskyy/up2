...
import { Sattellites } from "../../src/Sattellites";

export function App() {
  const beepAudioRef = useRef(new Audio(radarBeep));
  ...
  useEffect(() => {
    ...
    if (mapContRef.current) {
      observer.observe(mapContRef.current);
    }
  ...
  useEffect(() => {
    ...
    Object.keys(markersRef.current).forEach((satName) => {
      if (!nearSatBase.find((sat) => sat.satName === satName)) {
        ...
        setSatColors((prev) => prev.filter(([name]) => name !== satName));
      }
    });
    ...
    nearSatBase.forEach(({ satName, nadirLat, nadirLon }) => {
      ...
      if (markersRef.current[satName]) {
        ...
        markersRef.current[satName].setIcon(
          L.divIcon({ className: dotClass, iconSize: [20, 20] })
        );
      } else {
        const newMarker = L.marker([nadirLat, nadirLon], {
          icon: L.divIcon({ className: dotClass, iconSize: [20, 20] }),
        }).addTo(map);
        markersRef.current[satName] = newMarker;
 ...
 function MapMode() {
  ...
  if (leafletMapRef.current) {
    leafletMapRef.current.setView([20, 0], 2); // ← Reset center and zoom
